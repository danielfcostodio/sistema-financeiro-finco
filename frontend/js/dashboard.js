...
function atualizarMillerOrr(dados) {
    ...
    document.getElementById('miller-min').textContent = `Mínimo: ${formatarMoeda(dados.miller_orr_minimo)}`;
    document.getElementById('miller-ret').textContent = `Retorno: ${formatarMoeda(dados.miller_orr_retorno)}`;
    document.getElementById('miller-max').textContent = `Máximo: ${formatarMoeda(dados.miller_orr_maximo)}`;
    
    // Calcular posição do marcador
    const range = dados.miller_orr_maximo - dados.miller_orr_minimo;
    let posicao = ((dados.saldo_atual - dados.miller_orr_minimo) / range) * 100;
...
async function carregarGraficoEntradasSaidas() {
    ...
    try {
        ...
        // Apenas BAIXADAS para análises e gráficos (fluxo real)
        const lancamentosAtivos = lancamentos.filter(l => l.situacao === 'BAIXADA');
...
async function carregarGraficoDespesasTipo() {
    ...
    try {
        ...
        // Apenas BAIXADAS para análises (fluxo real)
        const lancamentosAtivos = lancamentos.filter(l => l.situacao === 'BAIXADA');
...
async function carregarTopFornecedores() {
    ...
    try {
        const lancamentos = await getLancamentos({ ano: ano, tipo: 'SAIDA', limit: 10000 });
        const lancamentosAtivos = lancamentos.filter(l => l.situacao === 'BAIXADA' && l.item);
