...
async function criarBackupManual() {
    ...
    statusDiv.innerHTML = '<span style="color: var(--cor-info);">⏳ Criando backup, aguarde...</span>';
    ...
    try {
        ...
        if (resultado.success) {
            const tamanhoKB = Math.round(resultado.original_size / 1024);
            statusDiv.innerHTML = `
                <span style="color: var(--cor-sucesso);">
                    ✅ Backup realizado com sucesso!<br>
                    <small>
                        📁 Arquivo: ${resultado.s3_key}<br>
                        📊 Tamanho: ${tamanhoKB} KB<br>
                        🕐 Data/Hora: ${resultado.timestamp}
                    </small>
                </span>
            `;
            mostrarAlerta('Backup realizado com sucesso!', 'sucesso');
        } else {
            throw new Error(resultado.detail || 'Erro no backup');
        }
        
    } catch (error) {
        console.error('Erro no backup:', error);
        statusDiv.innerHTML = `<span style="color: var(--cor-perigo);">❌ Erro: ${error.message}</span>`;
        mostrarAlerta('Erro ao criar backup: ' + error.message, 'erro');
    } finally {
        btn.disabled = false;
...
async function verificarStatusBackup() {
    const statusDiv = document.getElementById('backup-status');
    statusDiv.style.display = 'block';
    statusDiv.innerHTML = '<span style="color: var(--cor-info);">⏳ Verificando status...</span>';
    ...
    try {
        ...
        if (status.enabled) {
            const awsOk = status.aws_configured ? '✅' : '❌';
            const bucketOk = status.bucket_configured ? '✅' : '❌';
            
            statusDiv.innerHTML = `
                <span style="color: var(--cor-sucesso);">
                    📊 <strong>Status do Backup S3</strong><br>
                    <small>
                        ${awsOk} AWS Configurado: ${status.aws_configured ? 'Sim' : 'Não'}<br>
                        ${bucketOk} Bucket: ${status.bucket_name}<br>
                        ⏰ Backup automático: A cada 1 hora
                    </small>
                </span>
            `;
        } else {
            statusDiv.innerHTML = `<span style="color: var(--cor-alerta);">⚠️ Sistema de backup não configurado</span>`;
        }
