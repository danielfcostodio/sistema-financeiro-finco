...
def auto_restore_on_startup():
    """
    Restaura automaticamente o banco do S3 ao iniciar
    SEMPRE restaura se houver backup disponível
    """
    logger.info("Verificando backup no S3...")
    
    try:
        s3 = get_s3_client()
        s3_key = f"{S3_KEY_PREFIX}/backup_latest.db.enc"
        
        # Verificar se backup existe no S3
        try:
            response = s3.get_object(Bucket=S3_BUCKET, Key=s3_key)
        except ClientError as e:
            if e.response['Error']['Code'] == 'NoSuchKey':
                logger.info("Nenhum backup encontrado no S3. Usando banco local.")
                return {"success": True, "message": "Sem backup no S3, usando banco local"}
            raise
        ...
        metadata = response.get('Metadata', {})
        logger.info(f"Banco restaurado do S3: {len(db_data)} bytes")
        ...
        logger.error(f"Erro na restauração automática: {e}")
        return {
            "success": False,
            "error": str(e)
        }
